...
-- Q12) Display all the names(First Name and Last name with a underscore in between and set the column name as Full Name)
...
-- (you only need to show the department code and the number of employees).
 select * from employees
 select Department, count(Name) as No_of_Emp from employees group by department;
